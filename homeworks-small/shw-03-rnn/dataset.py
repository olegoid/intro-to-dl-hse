...
class TextDataset(Dataset):
    ...
    def __init__(self, data_file: str, train: bool = True, sp_model_prefix: str = None, vocab_size: int = 2000,
                 normalization_rule_name: str = 'nmt_nfkc_cf', model_type: str = 'bpe', max_length: int = 128):
        """
        ...
        # split texts to train and validation
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, int]:
        ...
        """
        indices = [self.bos_id] + self.sp_model.encode(self.texts[item]) + [self.eos_id]
        length = min(len(indices), self.max_length)
        indices = indices[:length] + [self.pad_id] * (self.max_length - length)
